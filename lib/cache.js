...
module.exports = function (opts) {
  ...
  var cache
    ...
    , options = extend(
    {
      ...
      , returnOldValueIfOutdated: false // return the outdated value if the value has expired, otherwise: undefined
  ...
  // allow disable garbage collection cleaning
    ...
    get: function (key) {
      ...
      if (item) {
        if ((item.expire) && (item.expire < Date.now())) {
          expire(key, item);
          return !options.returnOldValueIfOutdated ? undefined : item.value;
        }
